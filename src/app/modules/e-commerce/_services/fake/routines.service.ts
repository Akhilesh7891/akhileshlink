...
export class RoutinesService extends TableService<Routine> implements OnDestroy {
  ...
  // READ
  find(tableState: ITableState): Observable<TableResponseModel<Routine>> {
    return this.http.get<Routine[]>(this.API_URL).pipe(
      map((response: Routine[]) => {
        const filteredResult = baseFilter(response, tableState);
        const result: TableResponseModel<Routine> = {
