...
export class PaymentTermsService extends TableService<PaymentTerm> implements OnDestroy {
  ...
  // READ
  find(tableState: ITableState): Observable<TableResponseModel<PaymentTerm>> {
    return this.http.get<PaymentTerm[]>(this.API_URL).pipe(
      map((response: PaymentTerm[]) => {
        const filteredResult = baseFilter(response, tableState);
        const result: TableResponseModel<PaymentTerm> = {
  ...
  updateStatusForItems(ids: number[], status: number): Observable<any> {
    return this.http.get<PaymentTerm[]>(this.API_URL).pipe(
      map((PaymentTerms: PaymentTerm[]) => {
        return PaymentTerms.filter(c => ids.indexOf(c.id) > -1).map(c => {
          c.status = status;
          return c;
        });
      }),
      exhaustMap((PaymentTerms: PaymentTerm[]) => {
        const tasks$ = [];
        PaymentTerms.forEach(PaymentTerm => {
          tasks$.push(this.update(PaymentTerm));
        });
        return forkJoin(tasks$);
      })
    );
